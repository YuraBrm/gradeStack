 ...
 * @author earl
 */
public class studentAndTaskList extends Application {

    
           static Parent GUIRoot;
           static Stage stage;
    ...
    //studentGUI, taskGUI, expectationScreen, and gradeScreen are labels that indicate
    //which section the user is in.
    Label studentGUI;
    Label taskGUI;
    Label expectationScreen;
    Label gradeScreen;
    ...
    //isInStudentSection is a boolean which dictates if the user is in the student section or not.
    //It is used in the backButton method to figure out what the back button should do when it is pressed.
...
//        GUIRoot = FXMLLoader.load(getClass().getResource("studentFXML.fxml"));
//
//       GUIRoot.getStylesheets().add(getClass().getResource("newCascadeStyleSheet.css").toString());
        ...
        studentGUI = new Label("Students");
        grid.add(studentGUI, 0, 1);
        studentGUI.setVisible(false);
        
        taskGUI = new Label("Tasks");
        grid.add(taskGUI, 0, 1);
        taskGUI.setVisible(false);
        
        expectationScreen = new Label("Expectations");
        grid.add(expectationScreen, 0, 1);
        expectationScreen.setVisible(false);
        
        gradeScreen = new Label("Grades");
        grid.add(gradeScreen, 0, 1);
        gradeScreen.setVisible(false);  
...
//grid.add(deleteStudent, 2, 4);
              ...
              //and adds them to kidButtons.
                try {
                    ...
                    //numberOfStudents is set back to 0 for the purpose of adding the students to kidButtons.
                    numberOfStudents = 0;
...
//}
        ...
        graphStudent.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(final ActionEvent event) {
                ...
                yAxis.setCategories(FXCollections.<String>observableArrayList(Arrays.asList(grades)));
                yAxis.setAutoRanging(false);
                yAxis.invalidateRange(Arrays.asList(grades));
                xAxis.setLabel("Tasks");
                yAxis.setLabel("Grades");

                //The graph is initialized, with the title being set and the series being created.
                lineChart = new LineChart<String, String>(xAxis, yAxis);
                        ...
                        try {
                            ...
                            while (expectationFileSearch != null) {
                                try {
                                    ...
                                    //Each grade is checked to see if it belongs to the student;
                                    while (gradeFileSearch != null) {
                                        if (gradeFileSearch.substring(0, studentForGraphing.length()).equals(studentForGraphing)) {
        ...
        addGrade.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if (MarkManagerFile.checkGrade2(newGrade) == 0) {
                    ...
                    //Here, the program searches for the student who was created to see if he or she already exists.
                    IO.openInputFile("students.txt");
                    BufferedReader findStudent = new BufferedReader(new FileReader("students.txt"));
                    String fullStudentString = findStudent.readLine();
                        
                    //studentExists is the boolean that becomes true if the student already exists.
            ...
            public void handle(ActionEvent event) {
                ...
                if (isInStudentSection == false) {
                    ...
                    if (addGrade.isVisible() == true) {
                        ...
                        //expectationLine is reset to add in the expectations later, and the grades are removed from the screen.
                        expectationLine = 1;
                        ...
                        //gradeLine is reset for adding in the grades if the user goes back to the grade screen, and the expectations are added to the screen.
                        gradeLine = 1;
                        addButtons = 0;
                        ...
                        gradeScreen.setVisible(false);
                        expectationScreen.setVisible(true);
                    ...
                    if (graphStudent.isVisible() == true) {
                        ...
                        //gradeLine is reset to add the expectations later and the grades are removed from the screen.
                        gradeLine = 1;
                        while (addButtons < gradeLabels.length) {
                            grid.getChildren().remove(gradeLabels[addButtons]);
                            addButtons++;
                        ...
                        //studentLine is reset to add the expectations later and the students are added to the screen.
                        studentLine = 1;
                        addButtons = 0;
        ...
        StackPane root = new StackPane();
                ...
                Rectangle mainSetting = new Rectangle(300, 300, 1200, 1200);
        mainSetting.setFill(Color.RED);
        root.getChildren().add(mainSetting);
    ...
    //switchToGrades is the ActionEvent which switches to the grades of a student or expectation when the student or expectation is clicked.
